...
校卡只读客户端遇到 Token 过期时会清除内存 access token/临时 Cookie，等待相同的根 CAS Session 刷新，重新获取校卡 Token，然后只重试查询一次。CMB 首次预热及 WebView 明确跳回学校登录 URL 时、网费只读预热/查询和生产支付客户端都复用同一协调器；不会各自竞争重新登录。CMB WebView 自动恢复也限制为一次。生产支付的建单和最终提交绝不因刷新 Session 被自动重复发送。
