...
| 当前焦点 | 真机提示验证码图片获取失败：对齐 Android 的原生 `/remind/authcode` GET，复用隐藏 WebView 校方 Cookie 并将返回 Cookie 带入后续登录；授权账号真机复验待用户完成 |
...
| 2026-09-24 | CARD-007 / AUTH-019 | 真机 Toast 明确失败于获取校方验证码图片；隐藏 WebView 继续建立同域会话，图片改为带该会话 Cookie 的原生 GET，禁止跟随重定向并合并响应 Cookie，再将图片送远端 OCR。失败 Toast 细分为缺少会话、网络、重定向、HTTP 状态和非图片 | Android `develop` 的 `/remind/authcode` 原生 GET 契约；URLProtocol 本地取图与 Cookie 测试、`git diff --check`；macOS CI/授权真机待验证 | iOS 待提交 |
