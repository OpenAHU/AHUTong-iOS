...
| 2026-09-23 | AUTH-016 | 修复可见 ADWMH “绑定账号”点按无反馈：对校方页面的空 `javascript:` 链接移除无效导航目标，仍由原 jQuery 点击脚本提交；转发可信主框架的网页 `alert` 到系统提示，并对提交中、Ajax 失败与超时给出可见反馈；隐藏自动登录遇到校方弹窗立即降级可见页 | `CampusWebAuthenticationTests` 新增可信来源提示策略测试；Windows 静态与 `git diff --check`，macOS CI 和授权账号真机绑定待验证 | iOS 待提交 |
