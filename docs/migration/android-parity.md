...
| 当前焦点 | 验证 ADWMH 可见验证码与 Android `develop` 同款 `/user/login`（`flag=0`）会话、后续远端识别自动续期及校园卡 Cookie 持久化的 macOS/Simulator/真机行为 |
...
| 严格完成定义 | 19 / 25（76.0%）；AUTH-02 的校园服务自动续期仍待授权账号真机验证，PAY-01～05 待授权账号/物理 iPhone 证据；CARD-01 与 CONTENT-01/02 的本轮 macOS 回归已通过 |
...
| CARD-01 | 校园卡余额与付款码 | `home/CampusCard.kt`、`AHURepository.kt`、`TokenManager.kt` | `Features/CampusCard/` | P4 / AUTH-02 | 已完成 | 首页余额和展开二维码面板自动读取均静默；用户点“获取付款码”或重试后才进入 ADWMH 续期。首次显示可见页手填验证码，既有 ADWMH Cookie 时先尝试隐藏 WebView + 远端 OCR，失败转可见页。成功读取后将更新 Cookie 写入 Keychain，收起/放大二维码不重复触发认证 | `CampusCardInteractionTests`、付款码 UI smoke 在 CI `35867866032` 通过；真实自动验证码与会话恢复仍计入 AUTH-02 真机门槛 | 2026-09-23 |
