...
| 2026-08-31 | REL-001 | 将 App 与 Widget 的 `MARKETING_VERSION` 统一设为 `1.0.0`，构建号继续保持 `1`；本次仅调整客户端版本配置，不执行上传或商店发布 | `project.yml` 双 target 版本字段静态复核，macOS/Xcode 构建待执行 | 本提交 |
