...
远端 OCR 诊断证据 `E-20260924-03`：#121 真机反馈已越过验证码图片获取，进入 `openahu.org/ocr/captcha` 阶段；Android `develop` 同样使用此 HTTPS 端点和 JSON `result` 字段，但 iOS 原请求仅等待 10 秒并统一宣称 JPEG。iOS 改为按图片签名选择上传 MIME、JPEG 对齐 Android 的 `image/jpg`，OCR 请求最长等待 20 秒且隐藏总流程仍限 30 秒；失败只报告超时、网络、重定向、HTTP 状态、响应格式或非四位结果，绝不显示图片或识别内容。`CampusCaptchaRecognizerTests` 使用 URLProtocol 本地夹具验证这些边界；macOS CI 和新包真机结论待验证。

本轮验证码取图修复证据 `E-20260924-02`：授权真机反馈隐藏续期在“获取验证码图片”阶段失败，旧 iOS 实现于校方页面执行 JavaScript `fetch`，而 Android `develop` 使用携带校园会话 Cookie 的原生 GET `/remind/authcode`。iOS 改为同域原生 GET、禁止跨域重定向、限制图片大小和格式、仅传当前校方 Cookie、合并响应 Cookie；远端 OCR 仍只接收图片且不附带凭据。`CampusCardCaptchaClientTests` 以 URLProtocol 本地夹具覆盖成功、Cookie、缺失会话、重定向、HTTP 失败和非图片；macOS CI 与真机结果待验证，不把旧 Toast 视为已确定的脚本策略根因。
...
| 2026-09-24 | CARD-007 / AUTH-019 | 真机 Toast 明确失败于获取校方验证码图片；隐藏 WebView 继续建立同域会话，图片改为带该会话 Cookie 的原生 GET，禁止跟随重定向并合并响应 Cookie，再将图片送远端 OCR。失败 Toast 细分为缺少会话、网络、重定向、HTTP 状态和非图片 | Android `develop` 的 `/remind/authcode` 原生 GET 契约；URLProtocol 本地取图与 Cookie 测试、`git diff --check`；macOS CI/授权真机待验证 | iOS `f250654` |
| 2026-09-24 | CARD-008 / AUTH-020 | #121 真机反馈显示验证码图片已成功获取，故障转移到远端 OCR；按图片格式选择 multipart MIME，JPEG 对齐 Android `image/jpg`，OCR 等待从 10 秒增至 20 秒、隐藏总超时为 30 秒，并细分安全错误 Toast | Android `develop` `AhuTong` 的 HTTPS `/ocr/captcha`、`result` 字段与 `image/jpg` 上传；URLProtocol 本地 OCR 失败分类测试；macOS CI 与真机待验证 | iOS 待提交 |
