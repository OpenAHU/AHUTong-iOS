...
| 严格完成定义 | 16 / 25（64.0%）；AUTH-02 仍待授权账号真机验证，CARD-01 与 CONTENT-01/02 的本轮行为调整待 macOS 回归，PAY-01～05 待授权账号/物理 iPhone 证据 |
...
| AUTH-02 | 登录、会话恢复、过期重登与退出 | `Login.kt`、`LoginViewModel.kt`、`AutoLoginInterceptor.kt`、`TokenAuthenticator.kt`、`sdk/*` | `Core/Auth/`、`Core/CampusCore/`、`Core/Networking/`、`Features/Login/` | P2 / D-003、D-004 | 待验证 | 隐私 v3 同意后以官方可见 WebView 首登，也可点“暂不登录”进入体验账户并跨启动保持；账密/Cookie 只入 ThisDeviceOnly Keychain；教务过期时前台隐藏 WebView single-flight 续期。ADWMH 首次由用户在可见页手填验证码；保存过该域 Cookie 后，用户主动使用校园服务触发的续期先以隐藏 WebView 从校方同会话获取验证码，再只把图片发往 `https://openahu.org/ocr/captcha` 识别并填入校方页面，失败降级可见页；写请求不重放 | `CampusSessionStoreTests` 覆盖跳过与恢复，`CampusWebAuthenticationTests`、`CampusCaptchaRecognizerTests` 覆盖首次授权门槛、严格 HTTPS OCR 边界和识别协议；本轮 macOS CI 与授权账号真机待验证 | 2026-09-23 |
...
| CARD-01 | 校园卡余额与付款码 | `home/CampusCard.kt`、`AHURepository.kt`、`TokenManager.kt` | `Features/CampusCard/` | P4 / AUTH-02 | 待本轮回归 | 首页余额和展开二维码面板自动读取均静默；用户点“获取付款码”或重试后才进入 ADWMH 续期。首次显示可见页手填验证码，既有 ADWMH Cookie 时先尝试隐藏 WebView + 远端 OCR，失败转可见页。成功读取后将更新 Cookie 写入 Keychain，收起/放大二维码不重复触发认证 | `CampusCardInteractionTests` 覆盖自动失败不拉登录和显式操作一次恢复；本轮 macOS CI 与授权账号真机待验证 | 2026-09-23 |
...
| CONTENT-01 | 失物招领只读 | `main/LostFound.kt`、`LostFoundViewModel.kt` | `Features/LostFound/` | P4 / AUTH-02 | 待本轮回归 | 自动读取真实 campus/type/list 端点时会话过期只显示缓存/登录按钮；点“登录校园服务以继续”才拉起 ADWMH WebView；分页、筛选和详情保留 | `CampusAuthenticatedClientTests` 覆盖自动读取未授权不触发刷新；本轮 macOS CI 与授权账号真机待验证 | 2026-09-23 |
| CONTENT-02 | 失物发布与删除 | 同上、`crawler/model/adwnh/*` | `Features/LostFound/Compose/` | P5 / CONTENT-01 | 待本轮回归 | 真实发布/删除端点只在服务端确认成功后改变 UI；会话过期显示失败与手动登录入口，不因自动重试触发 WebView 或重放写请求 | 既有所有权/发布/删除单测保留；本轮 macOS CI 待验证 | 2026-09-23 |
...
| 2026-09-23 | AUTH-014 / CARD-004 / CONTENT-004 | 登录页增加“暂不登录，使用体验账户”，已接受隐私的体验选择跨启动保持；校园卡余额/二维码及失物招领自动读取全部静默，只有显式“获取付款码”或“登录校园服务”拉起 ADWMH WebView；保存校园卡请求可能轮换的 Cookie | `CampusSessionStoreTests`、`CampusCardInteractionTests`、`CampusAuthenticatedClientTests` 与登录 UI smoke 已扩充；Windows `git diff --check`，macOS CI 与授权账号真机待验证 | iOS 待提交 |
| 2026-09-23 | AUTH-015 | 按已披露的“首次手填、后续自动”补齐 iOS ADWMH 续期：只在 Keychain 已保存该域 Cookie 后启用隐藏 WebView，从校方同会话读取当次验证码，经 `openahu.org` HTTPS multipart `captcha` 上传图片，验证四位结果后用参数化 JavaScript 填入校方表单；OCR、页面或校验失败降级可见登录，后台不创建 WebView | 新增 `CampusCaptchaRecognizerTests` 的域名/凭据隔离请求测试，`CampusWebAuthenticationTests` 的首次授权门槛测试；macOS CI、真机验证码与服务端保留规则待验证 | iOS 待提交 |
