...
| 2026-09-23 | AUTH-017 | 真机反馈“OPENID 获取失败”后对照 Android `develop` `e7d27ae` 纠正 ADWMH 入口：可见 WebView 保留校方验证码与同会话 Cookie，拦截原账号绑定点击，按 Android 字段 `username/pwd/flag=0/imgcode` 向校方 `/user/login` 提交；隐藏 OCR 续期也复用该接口，只有 JSON `code=10000` 才导出 Cookie 并由原生只读接口复验，完全避开 `/index/user/account/bind` | 新增本地 URLProtocol 契约测试与失败/缺会话路径；Windows 静态检查，macOS CI 和授权账号真机待验证 | iOS 待提交 |
