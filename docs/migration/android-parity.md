...
| CARD-01 | 校园卡余额与付款码 | `home/CampusCard.kt`、`AHURepository.kt`、`TokenManager.kt` | `Features/CampusCard/` | P4 / AUTH-02 | 待本轮回归 | 首页余额静默读取；展开二维码面板自动静默读取，过期且有历史 ADWMH Cookie 时先尝试隐藏 WebView + OCR，不弹登录页。用户在失败状态主动重试时才允许可见登录；登录成功事件与二维码恢复后立即静默刷新余额，避免旧失败状态继续显示 `¥ --`，较早请求不得覆盖新结果；内联和全屏二维码点按强制重新取码并刷新余额 | `CampusCardInteractionTests` 新增自动隐藏续期和登录后余额刷新的状态测试；二维码点击及隐藏续期阶段诊断见 `E-20260924-01`；macOS UI 复验与授权账号真机待验证 | 2026-09-24 |
...
| CONTENT-01 | 失物招领只读 | `main/LostFound.kt`、`LostFoundViewModel.kt` | `Features/LostFound/` | P4 / AUTH-02 | 已完成 | 自动读取真实 campus/type/list 端点时会话过期只显示缓存/登录按钮；点“登录校园服务以继续”才拉起 ADWMH WebView；只读接口接受校方 `10000` 成功码，避免把“操作成功!”当成错误；分页、筛选和详情保留 | `CampusAuthenticatedClientTests` 的静默未授权与既有失物测试在 CI `35867866032` 通过；`LostFoundTests` 的 `10000` 本地夹具已在 CI `35898534602` 通过，见 `E-20260924-01`；真实登录联调仍依赖 AUTH-02 真机验证 | 2026-09-23 |
...
| PREF-01 | 设置、偏好、关于、许可证与贡献者 | `Settings.kt`、`settings/*`、`PreferencesViewModel.kt`、`LicenseViewModel.kt` | `Features/Settings/` | P1→P7 | 待本轮回归 | 设置首页显示真实 App 图标和账户信息卡；登录真实账号/重新登录操作仍在首页，只有隐私开关与政策入口留在偏好设置，体验用户也可访问；Debug 页及隐藏入口已删除，贡献名单保留匿名展示名并补齐 Android `develop` 两位开发者 | 设置/体验账户 UI smoke 已更新，确认账户操作不再出现在偏好设置；macOS CI 待验证 | 2026-09-24 |
...
| 2026-09-24 | AUTH-018 / CARD-005 | 真机确认手动登录成功但余额仍旧、二维码展开仍需再点：修复 SDK 原生 NDJSON Cookie 存档误写 Keychain，改用扁平 Cookie 快照并在冷启动迁移旧格式；展开二维码自动静默读取与优先隐藏续期，只有显式失败重试可拉可见登录；登录成功事件立即刷新余额并防止旧请求覆盖新结果。WidgetKit 扩展已存在且包含在 IPA 中，无需标“开发中” | 新增旧快照迁移、自动隐藏续期、余额再次读取测试；Windows `git diff --check`，macOS CI 与授权账号真机待验证 | iOS 待提交 |
| 2026-09-24 | PREF-010 | 应用户要求纠正 `9c98624` 的设置分组：账户信息卡及“登录真实账号/重新登录”恢复到设置首页；偏好设置仅保留隐私选择与政策入口等偏好项，体验账户的同意后登录路径继续可用 | 设置与体验账户 UI smoke 新增首页账户入口和偏好页无账户入口断言；Windows `git diff --check`，macOS CI 待验证 | iOS 待提交 |
| 2026-09-24 | CARD-006 / CONTENT-005 | 校园卡隐藏续期失败时增加仅含阶段原因的临时测试 Toast，保留凭据并降级可见登录；二维码图像点击强制刷新付款码及余额；失物招领只读响应接受 Android 校园服务成功码 `10000`，写操作仍遵守原契约；旧版 Cookie 快照仅在识别到原生 NDJSON 且转换结果非空时迁移，避免离线快照被空数组覆盖 | 对照 Android `develop` `e7d27ae` 的二维码点击与 ADWMH 成功码；新增失物招领响应码、旧快照识别和二维码点击测试；CI `35898534602` 373 单测通过、二维码 UI 断言待修订复验；真机错误阶段需用户验证 | iOS 待提交 |
