...
| 当前里程碑 | iOS 登录已改为官方可见 WebView 首登、前台隐藏 WebView 续期和版本化隐私决策；接受或拒绝隐私后均可选择只含课表与设置的体验账户 |
| 当前焦点 | 原生取图后真机已进入远端 OCR 阶段；对齐 Android 的上传 MIME 与请求头、延长 OCR 等待并将超时/HTTP/响应格式/非四位结果分开诊断，授权账号真机仍待复验 |
| 下一步 | 物理 iPhone 使用授权账号验证首登、Cookie 过期、ADWMH 自动验证码阶段 Toast、失物招领列表和二维码点击刷新；失败时只反馈脱敏提示文字 |
| 客户端实现覆盖 | 25 / 25（100%）；不再将教学评价计入当前 iOS 产品范围 |
| Android 生产行为对齐 | 25 / 25（100%）；仅统计当前 iOS 保留的功能切片 |
| 严格完成定义 | 17 / 25（68.0%）；AUTH-02 的校园服务自动续期仍待授权账号真机验证，CARD-01 与 PREF-01 本轮调整待 macOS 回归，PAY-01～05 待授权账号/物理 iPhone 证据 |
...
| AUTH-02 | 登录、会话恢复、过期重登与退出 | `Login.kt`、`LoginViewModel.kt`、`AutoLoginInterceptor.kt`、`TokenAuthenticator.kt`、`sdk/*` | `Core/Auth/`、`Core/CampusCore/`、`Core/Networking/`、`Features/Login/` | P2 / D-003、D-004 | 待真机验证 | 隐私 v3 同意后以官方可见 WebView 首登，也可点“暂不登录”进入体验账户并跨启动保持；账密/Cookie 只入 ThisDeviceOnly Keychain；教务过期时前台隐藏 WebView single-flight 续期。ADWMH 首次在可见 WebView 手填验证码，提交按 Android `develop` 契约直达校方 `/user/login`（`flag=0`）；后续由隐藏 WebView 建立校方会话，再以同域原生 GET 从校方同会话获取验证码，只将图片发往 `https://openahu.org/ocr/captcha` 识别并用同一 `/user/login` 续期，失败降级可见页。Keychain 会话快照统一使用 SDK 的扁平 Cookie 列表，仅在识别为原生 NDJSON 且转换非空时迁移旧 Cookie 存档，离线缓存不被空数组覆盖；写请求不重放；隐藏续期失败暂显示脱敏阶段 Toast | `CampusCardLoginClientTests` 覆盖登录字段与失败响应，`CampusSessionStoreTests` 新增旧快照迁移；373 个单元测试已通过，旧快照离线保护见 `E-20260924-01`；最终 macOS CI `35901490865` 全绿，取图修复与 OCR 诊断见 `E-20260924-02`、`E-20260924-03`；授权账号真机仍待验证 | 2026-09-24 |
...
| CARD-01 | 校园卡余额与付款码 | `home/CampusCard.kt`、`AHURepository.kt`、`TokenManager.kt` | `Features/CampusCard/` | P4 / AUTH-02 | 待真机验证 | 首页余额静默读取；展开二维码面板自动静默读取，过期且有历史 ADWMH Cookie 时先尝试隐藏 WebView + OCR，不弹登录页。用户在失败状态主动重试时才允许可见登录；登录成功事件与二维码恢复后立即静默刷新余额，避免旧失败状态继续显示 `¥ --`，较早请求不得覆盖新结果；内联和全屏二维码点按强制重新取码并刷新余额 | `CampusCardInteractionTests` 新增自动隐藏续期和登录后余额刷新的状态测试；二维码点击及隐藏续期阶段诊断见 `E-20260924-01`；macOS UI 复验见 CI `35901490865`，验证码取图修复见 `E-20260924-02`；授权账号真机待验证 | 2026-09-24 |
...
| CONTENT-01 | 失物招领只读 | `main/LostFound.kt`、`LostFoundViewModel.kt` | `Features/LostFound/` | P4 / AUTH-02 | 已完成 | 自动读取真实 campus/type/list 端点时会话过期只显示缓存/登录按钮；点“登录校园服务以继续”才拉起 ADWMH WebView；只读接口接受校方 `10000` 成功码，避免把“操作成功!”当成错误；分页、筛选和详情保留 | `CampusAuthenticatedClientTests` 的静默未授权与既有失物测试在 CI `35867866032` 通过；`LostFoundTests` 的 `10000` 本地夹具已在 CI `35898534602` 通过，见 `E-20260924-01`；真实登录联调仍依赖 AUTH-02 真机验证 | 2026-09-24 |
...
| --- | --- | --- | --- |
| APP-01、PREF-01、OPS-01 | 已完成 | 系统 `TabView`/`UITabBar`、UIKit 原生导航栏、双原生返回；账户/隐私/登录迁入偏好设置，Debug 页面删除；更新、反馈、许可证、主题与 Dynamic Type 保留 | 原生导航与底栏见 `E-20260716-01`；本轮设置 UI smoke 与截图在 CI `35856636143` 通过 |
