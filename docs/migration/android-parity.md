...
| 当前焦点 | 验证校园卡隐藏续期阶段 Toast、二维码点按刷新、失物招领 `10000` 成功码及旧 Cookie 存档迁移；ADWMH 真机联调仍待授权账号 |
...
| CARD-01 | 校园卡余额与付款码 | `home/CampusCard.kt`、`AHURepository.kt`、`TokenManager.kt` | `Features/CampusCard/` | P4 / AUTH-02 | 待真机验证 | 首页余额静默读取；展开二维码面板自动静默读取，过期且有历史 ADWMH Cookie 时先尝试隐藏 WebView + OCR，不弹登录页。用户在失败状态主动重试时才允许可见登录；登录成功事件与二维码恢复后立即静默刷新余额，避免旧失败状态继续显示 `¥ --`，较早请求不得覆盖新结果；内联和全屏二维码点按强制重新取码并刷新余额 | `CampusCardInteractionTests` 新增自动隐藏续期和登录后余额刷新的状态测试；二维码点击及隐藏续期阶段诊断见 `E-20260924-01`；macOS UI 复验见 CI `35901490865`，授权账号真机待验证 | 2026-09-24 |
