...
| 当前焦点 | 验证首次登录跳过与体验状态恢复、ADWMH 自动读取静默降级、首次可见验证码与后续远端识别自动续期、校园卡 Cookie 持久化的 macOS/Simulator/真机行为 |
...
| AUTH-02 | 登录、会话恢复、过期重登与退出 | `Login.kt`、`LoginViewModel.kt`、`AutoLoginInterceptor.kt`、`TokenAuthenticator.kt`、`sdk/*` | `Core/Auth/`、`Core/CampusCore/`、`Core/Networking/`、`Features/Login/` | P2 / D-003、D-004 | 待真机验证 | 隐私 v3 同意后以官方可见 WebView 首登，也可点“暂不登录”进入体验账户并跨启动保持；账密/Cookie 只入 ThisDeviceOnly Keychain；教务过期时前台隐藏 WebView single-flight 续期。ADWMH 首次由用户在可见页手填验证码；保存过该域 Cookie 后，用户主动使用校园服务触发的续期先以隐藏 WebView 从校方同会话获取验证码，再只把图片发往 `https://openahu.org/ocr/captcha` 识别并填入校方页面，失败降级可见页；写请求不重放 | CI `35867866032` 通过 362 单测、10 UI；`CampusSessionStoreTests`、`CampusWebAuthenticationTests`、`CampusCaptchaRecognizerTests` 覆盖离线逻辑和 OCR 请求边界；授权账号真机仍待验证 | 2026-09-23 |
