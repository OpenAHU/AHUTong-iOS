...
| 当前焦点 | 验证旧版校园 Cookie 存档迁移、付款码展开自动静默读取及隐藏续期、登录成功后的余额刷新；ADWMH 真机联调仍待授权账号 |
...
9. 2026-09-23 用户要求彻底删除 iOS Debug 页面与 App 信息卡隐藏入口；设置首页使用真实 App 图标，账户、隐私和登录设置归入偏好设置。
...
| CARD-01 | 校园卡余额与付款码 | `home/CampusCard.kt`、`AHURepository.kt`、`TokenManager.kt` | `Features/CampusCard/` | P4 / AUTH-02 | 待本轮回归 | 首页余额静默读取；展开二维码面板自动静默读取，过期且有历史 ADWMH Cookie 时先尝试隐藏 WebView + OCR，不弹登录页。用户在失败状态主动重试时才允许可见登录；登录成功事件与二维码恢复后立即静默刷新余额，避免旧失败状态继续显示 `¥ --`，较早请求不得覆盖新结果 | `CampusCardInteractionTests` 新增自动隐藏续期和登录后余额刷新的状态测试；本轮 macOS CI 与授权账号真机待验证 | 2026-09-24 |
...
| CONTENT-01 | 失物招领只读 | `main/LostFound.kt`、`LostFoundViewModel.kt` | `Features/LostFound/` | P4 / AUTH-02 | 已完成 | 自动读取真实 campus/type/list 端点时会话过期只显示缓存/登录按钮；点“登录校园服务以继续”才拉起 ADWMH WebView；分页、筛选和详情保留 | `CampusAuthenticatedClientTests` 的静默未授权与既有失物测试在 CI `35867866032` 通过；真实登录联调仍依赖 AUTH-02 真机验证 | 2026-09-23 |
...
| 2026-09-24 | CARD-006 / CONTENT-005 | 校园卡隐藏续期失败时增加仅含阶段原因的临时测试 Toast，保留凭据并降级可见登录；二维码图像点击强制刷新付款码及余额；失物招领只读响应接受 Android 校园服务成功码 `10000`，写操作仍遵守原契约；旧版 Cookie 快照仅在识别到原生 NDJSON 且转换结果非空时迁移，避免离线快照被空数组覆盖 | 对照 Android `develop` `e7d27ae` 的二维码点击与 ADWMH 成功码；新增失物招领响应码、旧快照识别和二维码点击测试；真机错误阶段需用户验证 | iOS 待提交 |
