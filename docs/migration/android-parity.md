...
| 当前焦点 | 验证首次登录跳过与体验状态恢复、ADWMH 自动读取静默降级、手动登录触发和校园卡 Cookie 持久化的 macOS/Simulator/真机行为 |
...
| AUTH-02 | 登录、会话恢复、过期重登与退出 | `Login.kt`、`LoginViewModel.kt`、`AutoLoginInterceptor.kt`、`TokenAuthenticator.kt`、`sdk/*` | `Core/Auth/`、`Core/CampusCore/`、`Core/Networking/`、`Features/Login/` | P2 / D-003、D-004 | 待验证 | 隐私 v3 同意后以官方可见 WebView 首登，也可点“暂不登录”进入体验账户并跨启动保持；账密/Cookie 只入 ThisDeviceOnly Keychain；教务过期时前台隐藏 WebView single-flight 续期，ADWMH 仅由用户显式操作拉起可见 WebView；写请求不重放 | `CampusSessionStoreTests` 覆盖接受隐私后的跳过与恢复，`CampusWebAuthenticationTests` 覆盖捕获来源与时序；本轮 macOS CI 与授权账号真机证据待验证 | 2026-09-23 |
...
| CARD-01 | 校园卡余额与付款码 | `home/CampusCard.kt`、`AHURepository.kt`、`TokenManager.kt` | `Features/CampusCard/` | P4 / AUTH-02 | 待本轮回归 | 首页余额和展开二维码面板自动读取均静默；用户点“获取付款码”或重试后才允许拉起 ADWMH 登录。成功读取后将更新的 Cookie 写入 Keychain，收起/放大二维码不重复触发认证 | `CampusCardInteractionTests` 覆盖自动失败不拉登录和显式操作一次恢复；本轮 macOS CI 与授权账号真机待验证 | 2026-09-23 |
