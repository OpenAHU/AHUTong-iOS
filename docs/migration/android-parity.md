...
| 下一步 | macOS/Xcode 完整测试与未签名 IPA；物理 iPhone 用授权账号验证清缓存重登后首次 ADWMH 隐藏登录、网络失败重试、余额/失物招领及隐私 v4 再确认 |
...
单次手动登录改造待验证证据 `E-20260924-05`：用户明确要求只在 App 官方登录页手动输入一次，ADWMH 首次会话与后续过期一律在前台隐藏完成。iOS 移除“必须已有校园卡 Cookie”的门槛，隐藏 WebView 加载校方页面后可从空 Cookie 请求验证码并接收校方新会话 Cookie，再将当次图片送已披露的远端 OCR，向校方提交并用余额只读接口验证；只读校园卡/失物招领请求自动续期一次，写请求不自动重放。失败保留原凭据、展示脱敏原因并允许重试，不自动打开第二登录页。隐私政策升级 v4，原 v3 同意须重新确认；本地契约与 UI 回归、macOS CI、授权真机待执行。
...
| CARD-01 | 校园卡余额与付款码 | `home/CampusCard.kt`、`AHURepository.kt`、`TokenManager.kt` | `Features/CampusCard/` | P4 / AUTH-02 | 待真机验证 | 首页余额静默读取；展开二维码面板自动静默读取，首次无 ADWMH Cookie 或后续过期时均使用隐藏 WebView + OCR，不弹登录页。失败状态主动重试仍只走隐藏认证；隐藏认证成功及二维码恢复后立即静默刷新余额，避免旧失败状态继续显示 `¥ --`，较早请求不得覆盖新结果；内联和全屏二维码点按强制重新取码并刷新余额 | `CampusCardInteractionTests` 新增自动隐藏续期和登录后余额刷新的状态测试；二维码点击及隐藏续期阶段诊断见 `E-20260924-01`；macOS UI 复验见 CI `35901490865`，验证码取图修复见 `E-20260924-02`；首次无 Cookie 静默登录见 `E-20260924-05`；授权账号真机待验证 | 2026-09-24 |
...
| CONTENT-01 | 失物招领只读 | `main/LostFound.kt`、`LostFoundViewModel.kt` | `Features/LostFound/` | P4 / AUTH-02 | 待真机验证 | 自动读取真实 campus/type/list 端点时会话过期先尝试隐藏续期，失败保留缓存和重试按钮，不拉起 ADWMH 可见 WebView；只读接口接受校方 `10000` 成功码，避免把“操作成功!”当成错误；分页、筛选和详情保留 | `CampusAuthenticatedClientTests` 的静默未授权与既有失物测试在 CI `35867866032` 通过；`LostFoundTests` 的 `10000` 本地夹具已在 CI `35898534602` 通过，见 `E-20260924-01`；无感续期见 `E-20260924-05`；真实登录联调仍依赖 AUTH-02 真机验证 | 2026-09-24 |
...
| 2026-09-24 | CARD-009 / AUTH-021 | 用户明确将“一次手动输入”限定为 App 官方首次登录；取消校园服务首次需历史 Cookie/手填验证码的门槛，前台隐藏 WebView 从首次 ADWMH 请求起使用 Keychain 凭据、同域取图及远端 OCR，失败提示重试而不弹第二登录页。隐私政策升级 v4 并要求重新确认，读请求最多重试一次，写请求不自动重放 | 无 Cookie 建立验证码会话、纯策略、首屏余额/付款码、失物招领和隐私版本测试；macOS CI 与授权真机待验证 | iOS 待提交 |
