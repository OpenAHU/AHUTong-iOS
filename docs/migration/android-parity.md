...
| 2026-09-22 | AUTH-012 | 修复校园卡登录弹窗循环：首页校园卡余额 `.task` 改为静默会话策略，未授权时只显示缓存/占位；只有用户主动打开付款码等操作才允许弹出 ADWMH WebView；取消或失败后自动 `.task` 不重试，第二次未授权不重复发布登录通知 | `CampusCardInteractionTests` 覆盖被动余额静默与 QR 失败后仅用户强制重试；macOS/Xcode 待运行 | iOS 待提交 |
