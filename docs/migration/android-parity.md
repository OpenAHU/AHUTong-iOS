...
| 2026-07-15 | AUDIT-002 | 继续收口低优先级复查项：天气页首入定位、PhotoKit 校历保存、Dynamic Type 缩放、不可逆校园卡缓存键、统一脱敏日志实际接入，以及续期凭据拒绝后的全局登录回退 | CI `29389054333` 仅发现缓存键测试缺少 `@MainActor`；修复后 CI `29389222613` 的 132 单测全绿、左边缘返回通过，内容区域返回因一次系统手势波动失败；Demo 隔离定位弹窗并保留两坐标真实手势有限重试后等待复跑。IPA `29389222592`、Archive `29389222625` 已成功 | iOS `0754440`、`7590212`、待回填 |
