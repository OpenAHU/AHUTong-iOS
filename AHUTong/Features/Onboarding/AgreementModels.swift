...
enum AgreementDocument: String, CaseIterable, Codable, Hashable, Identifiable, Sendable {
    ...
    var body: String {
        ...
        case .privacy:
            ...
            2. 学号、密码和 Cookie 仅用于登录 one.ahu.edu.cn、jw.ahu.edu.cn 及必要的 adwmh.ahu.edu.cn 校园服务。首次登录需要图形验证码的校园服务时，验证码必须由您在可见页面中手动填写。首次授权成功后，只要您未撤回同意，后续 Cookie 过期时 App 可启动 WebView，自动填入本机保存的账密，并将当次校方验证码图片发送到安大通配置的远程验证码识别接口，使用返回的识别文本帮助填入验证码，以恢复会话。
